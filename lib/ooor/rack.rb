module Ooor
  ...
  class Rack
    ...
    def self.share_session!(env, status, headers, body)
      response = ::Rack::Response.new body, status, headers
      public_session = env['ooor']['public_session']
      if public_session.config[:session_sharing]
        if public_session.config[:username] == 'admin'
          if public_session.config[:force_session_sharing]
        ...
        session_id = public_session.web_session[:session_id]
        expiry = Time.now+24*60*6
        if public_session.web_session[:sid] #v7
          self.share_session_v7!(public_session, response, session_id, expiry)
        else #v8
    ...
    def self.set_ooor!(env)
      ...
      public_session = Ooor.session_handler.retrieve_session(Ooor.default_config, web_session)
      env['ooor'] = {'context' => context, 'public_session' => public_session} #TODO ooor_model, see OOOREST
    end

    protected

    def self.share_session_v7!(env, response, session_id, expiry)
      response.set_cookie("sid", {:value => public_session.web_session[:sid], :path => "/", :expires => expiry})
      ...
      response.set_cookie("last_used_database", {:value => public_session.config[:database], :path => "/", :expires => expiry})
    end
